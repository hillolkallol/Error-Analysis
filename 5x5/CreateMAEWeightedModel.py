...
dataset = Dataset("mae_weighted_model_5x5.nc", "w", format="NETCDF4")
...
MAE = pd.read_csv('MAE_70_5x5.csv', header=None)
...
for i in index30:
    ...
    for j in range(10):
        ...
        for k in range(1, 25):
            print(i, j, k)
            a = rain_models[i,j,k,:,:]
            b = np.array(pd.to_numeric(MAE[k+1], errors='coerce').fillna(0))[1:].reshape((229, 334))
            b = b + 1
            a[a > 300000] = np.nan
            b[b > 300000] = np.nan
            print(a)
