...
dataset = Dataset("rmse_weighted_model_5x5.nc", "w", format="NETCDF4")
dataset.set_auto_mask(False)

days = dataset.createDimension('days', 39)
time = dataset.createDimension('time', 20)
y_axis = dataset.createDimension('y_axis', 229)
x_axis = dataset.createDimension('x_axis', 334)
...
RMSE = pd.read_csv('RMSE_70_5x5.csv', header=None)
...
for i in index30:
    ...
    for j in range(10):
        # print('time:', j)
        numeratorSum = np.zeros(shape=(229, 334))
        denominatorSum = np.zeros(shape=(229, 334))
        for k in range(1, 25):
            print(i, j, k)
            a = rain_models[i,j,k,:,:]
            b = np.array(pd.to_numeric(RMSE[k+1], errors='coerce').fillna(0))[1:].reshape((229, 334))
            # b = b + 1
            a[a > 30000] = np.nan
            b[b > 30000] = np.nan
            # print(a)
